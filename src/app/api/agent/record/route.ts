...
// 辅助启发式分类器（包含用户专属映射）
function inferCategory(text: string): { category: Category; type: TransactionType; title?: string } {
  const t = text.toLowerCase();

  // 用户专属规则 1: TokenMP 项目支出
  if (t.includes("讯飞") || t.includes("iflytek") || t.includes("tokenmp") || t.includes("coding plan")) {
    return { category: "tokenmp_project", type: "expense" };
  }
  ...
  if (t.includes("初温")) {
    customTitle = "兰州拉面";
    return { category: "food", type: "expense", title: customTitle };
  }
  if (t.includes("禹莉")) {
    customTitle = "小宋面馆";
    return { category: "food", type: "expense", title: customTitle };
  ...
  if (t.includes("饭") || t.includes("餐") || t.includes("吃") || t.includes("外卖") || t.includes("咖啡") || t.includes("茶") || t.includes("肯德基") || t.includes("麦当劳") || t.includes("拉面") || t.includes("面馆")) return { category: "food", type: "expense" };
  ...
  if (t.includes("房租") || t.includes("物业") || t.includes("水电") || t.includes("电费") || t.includes("水费") || t.includes("燃气") || t.includes("酒店") || t.includes("民宿")) return { category: "housing", type: "expense" };
  if (t.includes("电影") || t.includes("游戏") || t.includes("玩") || t.includes("门票") || t.includes("旅游")) return { category: "entertainment", type: "expense" };
  if (t.includes("手机") || t.includes("电脑") || t.includes("数码") || t.includes("耳机") || t.includes("键盘") || t.includes("云服务") || t.includes("腾讯云") || t.includes("阿里云")) return { category: "digital", type: "expense" };
  ...
  if (t.includes("转账") || t.includes("借钱") || t.includes("请客")) return { category: "social", type: "expense" };
...
function inferAccount(text: string): AccountType {
  const t = text.toLowerCase();
  if (t.includes("微信") || t.includes("零钱") || t.includes("分付")) return "wechat";
  if (t.includes("支付宝") || t.includes("花呗")) return "alipay";
  if (t.includes("信用卡")) return "bank_credit";
  if (t.includes("银行卡") || t.includes("储蓄卡") || t.includes("农行") || t.includes("招行") || t.includes("工行") || t.includes("建行")) return "bank_debit";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (body.prompt || body.message || body.text) {
      ...
      let title = inferred.title;
      if (!title) {
        title = rawText
          .replace(/(\d+(?:\.\d+)?)\s*(?:元|块|rmb|¥)?/gi, "")
          .replace(/(微信支付|支付宝|银行卡|信用卡|现金|花呗|分付)/g, "")
          .replace(/(记一笔|帮我记|记账|花了|消费了|买了|支出)/g, "")
          .trim();
      }
